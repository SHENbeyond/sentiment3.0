...
path_out1 = open(path+"/shuchu_pro_sen_pmi.txt",'w')
path_out2 = open(path+"/shuchu_sen_deg_pmi.txt",'w')
path_pmi_all = open(path+"/pro_sen_pmi_corpus_sort1.txt")
    ...
    for next,line in enumerate(corpus):
        ...
        for parse_sentence in parse_line:
            ...
            for path_sen_pro_one in path_sen_pro:
                union = (parse_sentence[path_sen_pro_one[0]]['cont'],parse_sentence[path_sen_pro_one[-1]]['cont'])
                if union in pmi_all and pmi_all[union] > 0:
